    ...
    public class SongSheetList : LayeredSkin.Controls.LayeredListBox
    {
        ...
        private void closeopenedlist()
        {
            foreach (DuiBaseControl ix in Items)
            {
                if (ix.Name.Replace("list_", "") == OpenedList)
                {
                    ...
                    ((DuiPictureBox)ix.FindControl("more")[0]).Image = Properties.Resources.Arrowright0;
        ...
        private void More_MouseClick(object sender, DuiMouseEventArgs e)
        {
            if (((DuiBaseControl)sender).Tag.ToString() == "Close")
            {
                ...
                ((DuiPictureBox)sender).Image = Properties.Resources.todown0;
                int a = 0;
                for (int i = 0; i < Items.Count; i++)
                {
                    if (Items[i].Tag != null)
            ...
            else
            {
                if (OpenedList == ((DuiBaseControl)((DuiPictureBox)sender).Parent).Name.Replace("list_", ""))
                {
                    ...
                    ((DuiPictureBox)sender).Image = Properties.Resources.Arrowright0;
                    for (int i = 0; i < Items.Count; i++)
                    {
                        if (Items[i].Tag != null)
                        {
        ...
        public bool AddList(string name)
        {
            ...
            if (!Searched)
            {
                ...
                btn.NormalImage = Properties.Resources.music_lyrics0;
                btn.HoverImage = Properties.Resources.music_lyrics1;
                btn.PressedImage = Properties.Resources.music_lyrics1;
                ...
                if (name == "默认列表")
                {
                    more.Image = Properties.Resources.todown0;
                    more.Tag = "Open";
                }
                else
                {
                    more.Image = Properties.Resources.Arrowright0;
                    more.Tag = "Close";
                }
